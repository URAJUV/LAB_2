...
node new_node(int freq, char c, node a, node b)
{
	node n = pool + n_nodes++;
	if (freq != 0){
		n->c = c;			//assign the character to the character of the node (eventually a leaf)
...
// function to remove the node from queue
node qremove()
{
	int i, l;
	node n = q[i = 1];
...
// logic to decode the huffman tree 
void decode(FILE *fp_huffman,FILE *fp_out){
	int i=0,lim=0,j=0;
	char c;
	node n = q[1];

	fscanf(fp_huffman,"%d",&lim);			//get length of bit stream from header
	...
	for(i=0; i<lim; i++){
		...
		else 		n = n->left;            //else goto left
                if(n->c){                               //until leaf (node with a character) meets
                        putchar(n->c);                  //spit that character out and
                        fputc(n->c,fp_out);             //save character in file
                        n = q[1];                       //reset queue
                }
	...
	putchar('\n');
	if (q[1] != n) printf("garbage input\n");	//the last node should end with character which reset the que
}
...
int main(int argc, char* argv[]){
	...
	//import frequency table compressed from file and create the huffman tree
